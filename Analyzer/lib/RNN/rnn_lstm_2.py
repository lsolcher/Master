...
def train(data_path, train_data, vocabulary, num_layers, num_epochs, batch_size, model_save_name,
...
def load_data(model_path, counts, word_list, tokens, articles, counts_test, word_list_test, tokens_test,  counts_valid, word_list_valid, tokens_valid):
    counts = next(iter(counts.values()))
    word_list = next(iter(word_list.values()))
    tokens = next(iter(tokens.values()))

    counts_test = next(iter(counts_test.values()))
    word_list_test = next(iter(word_list_test.values()))
    tokens_test = next(iter(tokens_test.values()))

    counts_valid = next(iter(counts_valid.values()))
    word_list_valid = next(iter(word_list_valid.values()))
    tokens_valid = next(iter(tokens_valid.values()))


    # build the complete vocabulary, then convert text data to list of integers
    word_to_id = build_vocab(counts)
    train_data = file_to_word_ids(word_list, word_to_id)
    test_data = file_to_word_ids(word_list_test, word_to_id)
    valid_data = file_to_word_ids(word_list_valid, word_to_id)
    ...
    #train(model_path, train_data, vocabulary, num_layers=2, num_epochs=60, batch_size=20,
    #model_save_name='two-layer-lstm-medium-config-60-epoch-0p93-lr-decay-10-max-lr')
    trained_model = model_path + "\\two-layer-lstm-medium-config-60-epoch-0p93-lr-decay-10-max-lr-final"
    test(trained_model, test_data, reversed_dictionary, vocabulary)
